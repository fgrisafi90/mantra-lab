...
import { loadSimulations, saveSimulations, createSimulation, duplicateSimulation } from '../storage/simulationStorage.js';
...
function renderSimulator() {
  ...
  const clubs=[...new Set(playerCatalog.map(p=>p.club))].sort((a,b)=>a.localeCompare(b,'it'));
  const filtered=filterCatalog(playerCatalog,{search:simCatalogSearch,role:simCatalogRole,club:simCatalogClub});
  ...
  return `${hero('SIMULATORE D’ASTA','Prova la tua rosa','Crea rose demo, prova moduli e cambia giocatori senza modificare la rosa reale.')}
