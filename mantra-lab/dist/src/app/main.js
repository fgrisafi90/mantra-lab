...
function renderRosa() {
  ...
  const budgetSummary=summarizeBudget(budget,squad);
  ...
  <section class="budget-card card"><div class="section-head"><div><span class="eyebrow">BUDGET ROSA</span><h3>Gestione crediti</h3></div></div><form id="budget-form" class="budget-form"><label>Budget iniziale<input name="budget" type="number" min="0" step="1" inputmode="numeric" placeholder="Es. 500" value="${budgetSummary.budget??''}"></label><button class="primary-btn" type="submit">Salva budget</button></form></section>
...
function renderAsta() {
  ...
  const budgetSummary=summarizeBudget(budget,squad);
  return hero('ASTA','Simulatore rosa','Controlla subito coperture, criticità e flessibilità della tua rosa.') + `<section class="card"><div class="section-head"><div><span class="eyebrow">BUDGET ASTA</span><h3>${budgetSummary.remaining==null?'Imposta il budget nella Rosa':`${budgetSummary.remaining} crediti disponibili`}</h3></div></div><div class="squad-kpis"><article class="stat-card"><span>Budget iniziale</span><strong>${budgetSummary.budget??'—'}</strong><small>crediti</small></article><article class="stat-card"><span>Spesi</span><strong>${budgetSummary.spent}</strong><small>acquisti registrati</small></article><article class="stat-card accent"><span>Disponibili</span><strong>${budgetSummary.remaining??'—'}</strong><small>crediti residui</small></article></div></section><section class="card issue-list">${issues.map(i=>`<article class="issue ${i.severity}"><span>${i.severity==='critical'?'!':i.severity==='warning'?'△':'✓'}</span><p>${escapeHtml(i.message)}</p></article>`).join('')}</section>`;
}
...
function bindEvents() {
  ...
  app.querySelector('#budget-form')?.addEventListener('submit', e => { e.preventDefault(); const fd=new FormData(e.currentTarget); const raw=fd.get('budget'); try{ budget=raw===''?null:Number(raw); saveBudget(raw===''?null:budget); render(); }catch(err){ alert(err.message); } });
