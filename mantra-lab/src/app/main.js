...
function renderRosa() {
  ...
  ${renderPlayerComparison(playerCatalog,comparisonIds,playerStatsIndex,playerStatsDataset,getFormation(active==='simulatore'?currentSimulation().formationId:formationId))}
...
function bindEvents() {
  ...
  app.querySelectorAll('[data-compare-slot]').forEach(select=>select.addEventListener('change',e=>{
    ...
    render();
    app.querySelector(`[data-compare-slot="${select.dataset.compareSlot}"]`)?.focus({preventScroll:true});
  }));
  app.querySelector('#clear-comparison')?.addEventListener('click',()=>{comparisonIds=['','',''];render();app.querySelector('[data-compare-slot="0"]')?.focus({preventScroll:true});});
