...
function parseMantraListMarkdown(markdown){const players=[];let roles=[];for(const rawLine of String(markdown||'').split(/\r?\n/)){const line=rawLine.trim();const heading=line.match(/^##\s+Ruolo:\s+(.+?)\s+\(/i);if(heading){roles=heading[1].split('/').map(x=>x.trim()).filter(Boolean);continue;}if(!line.startsWith('|')||/^\|\s*(Nome|[-:]+)/i.test(line))continue;const cells=line.split('|').slice(1,-1).map(x=>x.trim());if(cells.length<4||!roles.length)continue;const [name,club,quotationRaw,fvmRaw]=cells;if(!name||!club||!/^\d/.test(String(quotationRaw)))continue;players.push({catalogId:`${catalogSlug(club)}:${catalogSlug(name)}`,name,club,roles:[...roles],quotation:Number(quotationRaw),fvm:Number(fvmRaw)});}const unique=new Map();for(const player of players)unique.set(player.catalogId,player);return [...unique.values()].sort((a,b)=>a.name.localeCompare(b.name,'it'));}
...
async function loadPlayerCatalog(){try{const response=await fetch(CATALOG_URL,{cache:'no-store'});if(!response.ok)throw new Error(`HTTP ${response.status}`);const players=parseMantraListMarkdown(await response.text());if(!players.length)throw new Error('Listone vuoto');return {players,error:null};}catch(error){return {players:[],error};}}
